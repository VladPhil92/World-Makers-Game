...
if (pkg.dependencies && Object.keys(pkg.dependencies).length) throw new Error('D1 must remain dependency-minimized until identity/store infrastructure is selected.');
...
for (const required of ['Content-Security-Policy', 'HttpOnly', 'SameSite=Strict', 'WORLD_MAKERS_ALLOW_DEMO_AUTH', 'WORLD_MAKERS_LAUNCH_SIGNING_SECRET', '/api/launch-context', '/api/avatar/loadout', '/api/selection', '/api/store/requests']) {
...
for (const mode of ['free-explore', 'missions', 'laboratory', 'cooperative']) {
  if (!catalog.includes(`id: '${mode}'`)) throw new Error(`Game mode missing from D1 catalog: ${mode}`);
}
if (!catalog.includes("playable: false")) throw new Error('Future cooperative mode must remain locked in D1.');
...
console.log('World Makers Player Dashboard D1 lint passed.');
